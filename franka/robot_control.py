...
from sensing.utils import skeletonize_img, find_length_and_endpoints, sort_skeleton_pts
import random
...
def rope_mask_info(image):
    ...
    cable_lower = np.array([25, 10, 180]) # morning (new rope)
    cable_upper = np.array([70, 255, 255]) # morning (new rope)
...
def grab_zed_mat(side_cam,runtime_parameters, image, point_cloud):
